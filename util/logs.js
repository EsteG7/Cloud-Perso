...
const logs = async (obj) => {
    ...
    const color = db.get(`color_${client.user.id}`) === null ? obj.client.config.color : db.get(`color_${client.user.id}`);
    const footer = db.get(`footer_${guild.id}`) === null ? obj.client.config.footer : db.get(`footer_${guild.id}`);
