    ...
    name: "embed",
     ...
     go: async (client, db, message, args, prefix, color, footer) => {
        ...
        const filter = m => message.author.id === m.author.id;
            ...
            .setEmoji("✅")
            .setLabel("Valider et envoyer dans un channel")
            const bt2 = new MessageButton()
            .setStyle("SUCCESS")
            .setCustomId("embed2"+message.id)
            .setEmoji("🔨")
            .setLabel("Envoyer ici")
            let menu = new MessageActionRow().addComponents(interactiveButtons)
            let butt = new MessageActionRow().addComponents(bt).addComponents(bt2)
             ...
             message.channel.send({embeds: [embedbase],
                ...
                client.on("interactionCreate", async (i) => {
                    ...
                    if (i.customId === "embed2" + message.id) {
                        i.deferUpdate()
                        let valider = await message.channel.send("Voulez-vous envoyer l'embed dans **ce salon** ?")
                        ...
                        message.channel.awaitMessages({
                            ...
                            if (lowercase === "oui" || lowercase === "yes" ||lowercase === "ce salon") {
                                await channel.send({ embeds: [embedbase] })
                                i.channel.send("Embed envoyée dans **ce salon**")
                            } else {
                                return message.channel.send("D'accord, l'opération a été annulée.").then((mm) => mm.delete({
                                    timeout: 2500
                                }))
                            }
                            ...
                            message.channel.send("**Désolé mais je ne peux pas modifier l'Image !**").then((mm) => mm.delete({
                              ...
                              case message.id + "Supprimer l'image":
                                ...
                                case message.id + "Modifier l'url du titre":
                                    i.deferUpdate()
                                    let Sown = await message.channel.send("Quel **URL** voulez-vous attribuez à l'embed ?")
